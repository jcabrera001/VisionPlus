...
Public Class frmParametrosGenerales
    ...
    End Sub
    Private Sub frmParametrosGenerales_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Tables("spMesAComparar '30201901'", "Periodo", cbxPeriodo, "EncabezadoCompleto", "PerEmpXCalendarioID", "PerEmpXCalendarioID")
            Tables("spPresupuestoZafra", "Categoria", cbxPresupuesto, "Descripcion", "PresupuestoID", "PresupuestoID")
    ...
    Private Sub cmdAceptar_Click(sender As Object, e As EventArgs) Handles cmdAceptar.Click
        ...
        frm.ClearError()
        If IsNothing(cbxPresupuesto.EditValue) Then frm.AddErr("Debe seleccionar el presupuesto a visualizar", 0)
        If IsNothing(cbxPeriodo.EditValue) Then frm.AddErr("Debe seleccionar el mes a consultar", 0)

        If frm.isErr Then
            frm.ShowDialog()
        Else
            prmList.Add(New SqlParameter("@presupuesto", cbxPresupuesto.EditValue.ToString))
            prmList.Add(New SqlParameter("@MesID", cbxPeriodo.EditValue.ToString))

            Dim frmRpt As New RptVisorCRFrm(parametros, prmList, New frmParametrosGenerales(parametros))
            frmRpt.Show()
            Me.Close()
        End If
    End Sub
